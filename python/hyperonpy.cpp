...
PYBIND11_MODULE(hyperonpy, m)
{
    ...
    m.def(
        "atom_sym", [](char const *name)
        { return CAtom(atom_sym(name)); },
        "Create symbol atom");
    m.def(
        "atom_var", [](char const *name)
        { return CAtom(atom_var(name)); },
        "Create variable atom");
    m.def(
        "atom_expr", [](py::list _children)
        {
    ...
    m.def(
        "atom_gnd", [](py::object object, CAtom ctyp)
        {
            atom_t* typ = atom_clone(ctyp.ptr);
            return CAtom(atom_gnd(new GroundedObject(object, typ))); },
        "Create grounded atom");
    m.def(
        "atom_free", [](CAtom atom)
        { atom_free(atom.ptr); },
        "Free C atom");

    m.def(
        "atom_eq", [](CAtom a, CAtom b) -> bool
        { return atom_eq(a.ptr, b.ptr); },
        "Test if two atoms are equal");
    m.def(
        "atom_to_str", [](CAtom atom)
        {
            std::string str;
    ...
    m.def(
        "atom_get_type", [](CAtom atom)
        { return atom_get_type(atom.ptr); },
        "Get type of the atom");
    m.def(
        "atom_get_name", [](CAtom atom)
        {
            std::string str;
    ...
    m.def(
        "atoms_are_equivalent", [](CAtom first, CAtom second)
    ...
    m.def(
        "vec_atom_new", []()
        { return CVecAtom(vec_atom_new()); },
        "New vector of atoms");
    m.def(
        "vec_atom_free", [](CVecAtom vec)
        { vec_atom_free(vec.ptr); },
        "Free vector of atoms");
    m.def(
        "vec_atom_size", [](CVecAtom vec)
        { return vec_atom_size(vec.ptr); },
        "Return size of the vector");
    m.def(
        "vec_atom_push", [](CVecAtom vec, CAtom atom)
        { vec_atom_push(vec.ptr, atom_clone(atom.ptr)); },
        "Push atom into vector");
    m.def(
        "vec_atom_pop", [](CVecAtom vec)
        { return CAtom(vec_atom_pop(vec.ptr)); },
        "Push atom into vector");

    py::class_<CGroundingSpace>(m, "CGroundingSpace");
    m.def(
        "grounding_space_new", []()
        { return CGroundingSpace(grounding_space_new()); },
        "New grounding space instance");
    m.def(
        "grounding_space_free", [](CGroundingSpace space)
        { grounding_space_free(space.ptr); },
        "Free grounding space");
    m.def(
        "grounding_space_add", [](CGroundingSpace space, CAtom atom)
        { grounding_space_add(space.ptr, atom_clone(atom.ptr)); },
        "Add atom into grounding space");
    m.def(
        "grounding_space_remove", [](CGroundingSpace space, CAtom atom)
        { return grounding_space_remove(space.ptr, atom.ptr); },
        "Remove atom from grounding space");
    m.def(
        "grounding_space_replace", [](CGroundingSpace space, CAtom from, CAtom to)
        { return grounding_space_replace(space.ptr, from.ptr, atom_clone(to.ptr)); },
        "Replace atom from grounding space");
    m.def(
        "grounding_space_eq", [](CGroundingSpace a, CGroundingSpace b)
        { return grounding_space_eq(a.ptr, b.ptr); },
        "Check if two grounding spaces are equal");
    m.def(
        "grounding_space_len", [](CGroundingSpace space)
        { return grounding_space_len(space.ptr); },
        "Return number of atoms in grounding space");
    m.def(
        "grounding_space_get", [](CGroundingSpace space, size_t idx)
        { return CAtom(grounding_space_get(space.ptr, idx)); },
        "Get atom by index from grounding space");
    m.def(
        "grounding_space_query", [](CGroundingSpace space, CAtom pattern)
        {
            py::list results;
    ...
    m.def(
        "grounding_space_subst", [](CGroundingSpace space, CAtom pattern, CAtom templ)
        {
            py::list atoms;
    ...
    m.def(
        "tokenizer_new", []()
        { return CTokenizer(tokenizer_new()); },
        "New tokenizer");
    m.def(
        "tokenizer_free", [](CTokenizer tokenizer)
        { tokenizer_free(tokenizer.ptr); },
        "Free tokenizer");
    m.def(
        "tokenizer_clone", [](CTokenizer tokenizer)
        { tokenizer_clone(tokenizer.ptr); },
        "Clone tokenizer");
    m.def(
        "tokenizer_register_token", [](CTokenizer tokenizer, char const *regex, py::function constr)
        {
...
#define ADD_TYPE(t, d) .def_property_readonly_static( \
                           #t, [](py::object) { return CAtom(ATOM_TYPE_##t()); }, d " atom type")
    ...
    m.def(
        "metta_new", [](CGroundingSpace space, char const *cwd)
        { return CMetta(metta_new(space.ptr, cwd)); },
        "New MeTTa interpreter instance");
    m.def(
        "metta_free", [](CMetta metta)
        { metta_free(metta.ptr); },
        "Free MeTTa interpreter");
    m.def(
        "metta_space", [](CMetta metta)
        { return CGroundingSpace(metta_space(metta.ptr)); },
        "Get space of MeTTa interpreter");
    m.def(
        "metta_tokenizer", [](CMetta metta)
        { return CTokenizer(metta_tokenizer(metta.ptr)); },
        "Get tokenizer of MeTTa interpreter");
    m.def(
        "metta_run", [](CMetta metta, CSExprParser &parser)
        {
