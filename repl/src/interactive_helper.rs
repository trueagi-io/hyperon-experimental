...
use hyperon::metta::text::{SExprParser, ParseTokenType};
...
impl Highlighter for ReplHelper {
    ...
    fn highlight<'l>(&self, line: &'l str, _pos: usize) -> Cow<'l, str> {
        ...
        //Iterate over the tokens generated by the parser, coloring them appropriately
            ...
            loop {
                let result = parser.parse_with_visitor(&metta.metta.tokenizer().borrow(), |token| {
                    ...
                    match token.token_type {
                        ...
                        ParseTokenType::LeftoverText => {
                            style_sequence.push(ERROR_COLOR);
                        }
                        _ => { } // We don't do anything with the compound tokens, e.g. Expression & Atom
                    }

                    //See if we need to render this token with the "bracket blink"
                    if let Some((_matching_char, blink_idx)) = &blink_char {
                        if token.src_range.contains(blink_idx) {
                            style_sequence.push("1;7");
                        }
                    }

                    //Render the token to the buffer if it's one of the ones we choose to render
                    if token_type_should_render(token.token_type) {
                        ...
                        //Push the token itself to the buffer
                        colored_line.push_str(&line[token.src_range]);

                        //And push an undo sequence, if the token was stylized
                        if style_count > 0 {
...
fn token_type_should_render(token_type: ParseTokenType) -> bool {
    match token_type {
        ParseTokenType::Comment |
        ParseTokenType::Variable |
        ParseTokenType::OpenParen |
        ...
        ParseTokenType::Special |
        ParseTokenType::MiscSymbol |
        ParseTokenType::Whitespace |
        ParseTokenType::LeftoverText => true,

        ParseTokenType::Atom |
        ParseTokenType::Expression => false,
    }
}
